...
int strcasecmp_a(const char* s1, const char* s2)
{
    int len1 = strlen(s1);
    int len2 = strlen(s2);
    if (len1 != len2)
...
private:
    ...
    {
        return strcasecmp_a(item.name.c_str(), "Connection") == 0;
    }
    ...
    ParseResult consume(Request &req, const char *begin, const char *end)
    {
        while( begin != end )
        {
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    if( req.method == "POST" || req.method == "PUT" )
                    {
                        ...
                        if(strcasecmp_a(h.name.c_str(), "Content-Length") == 0 )
                        {
                            contentSize = atoi(h.value.c_str());
                            req.content.reserve( contentSize );
                        }
                        else if(strcasecmp_a(h.name.c_str(), "Transfer-Encoding") == 0 )
                        {
                            if(strcasecmp_a(h.value.c_str(), "chunked") == 0)
                                chunked = true;
                        }
                    }
                    state = ExpectingNewline_2;
            ...
            case ExpectingNewline_3: {
                ...
                {
                    if(strcasecmp_a(it->value.c_str(), "Keep-Alive") == 0 )
                    {
                        req.keepAlive = true;
                    }
                    else  // == Close
